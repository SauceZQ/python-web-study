...
class bqw_wx_formId(Model):
    ...
    is_push = IntegerField(column_type='int(2)', default=0)
    created_time = DateField(default=datetime.datetime.now())


class BqwWxPush(Model):
    __table__ = "wx_push"
    id = IntegerField(column_type='int(11)', primary_key=True)
    openid = StringField(column_type='varchar(100)')
    push_url = StringField(column_type='varchar(256)')
    novel_name = StringField(column_type='varchar(256)')
